...
class WebexRoomRAPTOR:
    ...
    def fetch_and_setup_conversation(self, key, room):
        with st.spinner("Gathering Webex Room Messages ...."):
            url = f"https://webexapis.com/v1/messages?roomId={room}&max=2500"
...
def page_chat():
    if 'chat_instance' not in st.session_state:
        ...
        st.session_state['chat_instance'] = WebexRoomRAPTOR(webex_details['key'], webex_details['room'], 'conversation.txt')
    ...
    user_input = st.text_input("Ask a question about the Webex Room Messages:")
...
# Streamlit UI setup
st.title("Routing Table RAPTOR")

# Initialize session state keys if they are not already set
if 'page' not in st.session_state:
    st.session_state['page'] = 'page_webex_room'
if 'conversation_history' not in st.session_state:
    st.session_state['conversation_history'] = ""

# Page routing
if st.session_state['page'] == 'page_webex_room':
    page_webex_room()
